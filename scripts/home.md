...
I ended up choosing [dygraphs](dygraphs.com) to display the data, due to its robust and interactive handling of large data sets. To dynamically load my data based on the desired range of datetimes, I turned to Josh Sanderson's excellent [demo](http://kaliatech.github.io/dygraphs-dynamiczooming-example/) of dynamically loading data to dygraphs, which I modified to use my DynamoDB tables as a data source. Using dygraphs also allowed me to tack on extras like using the [suncalc](https://github.com/mourner/suncalc) library to shade the nighttime sections of the graph (when the x-axis only spans 14 days or less). This allowed a robust visulization of the data from any point in the recording history: 
